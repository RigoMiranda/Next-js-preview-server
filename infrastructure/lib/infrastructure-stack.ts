...
export class InfrastructureStack extends cdk.Stack {
  constructor(scope: cdk.App, id: string, props?: cdk.StackProps) {
    ...
    /* Auto Scaling Group*/
    const autoScalingGroups = new EC2Stack(scope, `PreviewServerAutoScalingGroups${stage}`, {...props})

    const application = new ServerApplication(this, `PreviewServerDeploymentGroup${stage}`, {
      applicationName: `NextPreviewServer-${stage}`
    });

    new ServerDeploymentGroup(this, 'PreviewServerCodeDeployDeploymentGroup', {
      application: application,
      role: serviceRole,
      deploymentGroupName: 'PreviewServer',
      autoScalingGroups: [autoScalingGroups.ec2AutoScalingGroup],
